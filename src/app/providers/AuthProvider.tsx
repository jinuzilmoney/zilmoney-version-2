...
import type { AuthUser, SignUpData, SocialProvider } from '@/src/shared/types/auth';
...
interface AuthContextType {
  ...
  signOut: () => void;
  socialSignIn: (provider: SocialProvider) => Promise<{ success: boolean; error?: string }>;
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem(AUTH_STORAGE_KEY) || sessionStorage.getItem(AUTH_SESSION_KEY);
      if (stored) {
  ...
  const socialSignIn = useCallback(async (provider: SocialProvider): Promise<{ success: boolean; error?: string }> => {
