...
const COUNTRY_CODES = [
  ...
  { code: '+44', country: 'UK', flag: '\u{1F1EC}\u{1F1E7}', name: 'United Kingdom' },
  ...
  { code: '+971', country: 'AE', flag: '\u{1F1E6}\u{1F1EA}', name: 'United Arab Emirates' },
...
export function PhoneInput({
  ...
  const selectedCountry = COUNTRY_CODES.find(c => c.code === countryCode && c.country === 'US') ||
        ...
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            className={cn(
              ...
              compact ? 'h-11 min-h-[44px] px-3 rounded-lg' : 'h-12 min-h-[48px] px-4 rounded-xl',
              error ? 'border-red-500' : 'border-gray-200 hover:border-gray-300 active:border-[#20319D]',
              'focus:outline-none focus:ring-2 focus:ring-[#20319D]'
            )}
          >
            <span className={compact ? 'text-lg' : 'text-xl'}>{selectedCountry.flag}</span>
            <span className={cn('font-medium text-gray-700', compact ? 'text-sm' : 'text-base md:text-sm')}>{selectedCountry.code}</span>
            <ChevronDown size={compact ? 16 : 18} className={cn('text-gray-400 transition-transform', isOpen && 'rotate-180')} />
          </button>
                  ...
                  filteredCountries.map((country, index) => (
                    <button
                      ...
                      className={cn(
                        ...
                        country.code === countryCode && country.country === selectedCountry.country && 'bg-[#20319D]/5'
                      )}
                    >
                      <span className="text-xl">{country.flag}</span>
                      <span className="text-base md:text-sm font-medium text-gray-900 flex-1">{country.name}</span>
                      <span className="text-base md:text-sm text-gray-500">{country.code}</span>
                    </button>
                  ))
                ) : (
