...
import {
  Search, Bell, ChevronDown, Menu, Check, User, Shield, ShieldCheck,
  Settings, LogOut, HelpCircle, MessageSquare, Monitor, BookOpen,
  Video, Lightbulb, AlertCircle, Heart, Calendar, MessageCircle,
  Phone, Building2, Plus, Mail, DollarSign, Sun, Moon, Zap, X,
} from 'lucide-react';
...
export const Header = ({ onMenuClick }: HeaderProps) => {
  ...
  const userTypeOptions: Array<{ type: UserType; label: string; badge: string; icon: React.ElementType; description: string }> = [
    { type: 'new', label: 'New User', badge: 'Unverified', icon: User, description: 'Basic access' },
    { type: 'l1-verified', label: 'L1 Verified User', badge: 'Level 1', icon: Shield, description: 'Standard verification' },
    { type: 'l2-verified', label: 'L2 Verified User', badge: 'Level 2', icon: ShieldCheck, description: 'Full access' },
  ];
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-2 sm:gap-4 flex-1">
            ...
            <div className="relative hidden md:block max-w-md w-full">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-500 pointer-events-none z-10" size={18} />
              <input
                type="text"
          ...
          <div className="flex items-center gap-1.5 sm:gap-3">
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-medium text-amber-700 dark:text-amber-300">Trial Ends in 30 Days</span>
              </div>
                  ...
                  >
                    <div className="p-4 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900">
                      <div className="flex items-center gap-2">
                        <HelpCircle size={18} className="text-[#20319D] dark:text-[#6B8AFF]" />
                        <h3 className="text-sm font-semibold text-gray-900 dark:text-slate-100">Help & Support</h3>
                      </div>
                    </div>
                    <div className="p-2">
                      <button onClick={() => setShowHelpMenu(false)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-all text-left">
                        <MessageSquare size={16} className="text-gray-600 dark:text-slate-400" />
                        <span className="text-sm text-gray-700 dark:text-slate-300">Live Chat</span>
                      </button>
                      <button onClick={() => setShowHelpMenu(false)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-all text-left">
                        <Monitor size={16} className="text-gray-600 dark:text-slate-400" />
                        <span className="text-sm text-gray-700 dark:text-slate-300">Live Screen Sharing</span>
                      </button>
                      <div className="border-t border-gray-200 dark:border-slate-700 my-1" />
                      <button onClick={() => { router.push('/help'); setShowHelpMenu(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-all text-left">
                        <BookOpen size={16} className="text-gray-600 dark:text-slate-400" />
                        <span className="text-sm text-gray-700 dark:text-slate-300">Help / Tutorial</span>
                      </button>
                      <button onClick={() => setShowHelpMenu(false)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-all text-left">
                        <Video size={16} className="text-gray-600 dark:text-slate-400" />
                        <span className="text-sm text-gray-700 dark:text-slate-300">Video Tutorial</span>
                      </button>
                      <div className="border-t border-gray-200 dark:border-slate-700 my-1" />
                      <button onClick={() => setShowHelpMenu(false)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-all text-left">
                        <Lightbulb size={16} className="text-gray-600 dark:text-slate-400" />
                        <span className="text-sm text-gray-700 dark:text-slate-300">Submit an Idea</span>
                      </button>
                      <button onClick={() => setShowHelpMenu(false)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-all text-left">
                        <AlertCircle size={16} className="text-gray-600 dark:text-slate-400" />
                        <span className="text-sm text-gray-700 dark:text-slate-300">Report an Issue</span>
                      </button>
                      <button onClick={() => setShowHelpMenu(false)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-all text-left">
                        <Heart size={16} className="text-gray-600 dark:text-slate-400" />
                        <span className="text-sm text-gray-700 dark:text-slate-300">Our Compliance</span>
                      </button>
                      <div className="border-t border-gray-200 dark:border-slate-700 my-1" />
                      <button onClick={() => setShowHelpMenu(false)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-all text-left">
                        <Calendar size={16} className="text-gray-600 dark:text-slate-400" />
                        <span className="text-sm text-gray-700 dark:text-slate-300">Book a Demo</span>
                      </button>
                      <button onClick={() => setShowHelpMenu(false)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-all text-left">
                        <MessageCircle size={16} className="text-gray-600 dark:text-slate-400" />
                        <span className="text-sm text-gray-700 dark:text-slate-300">Text Us</span>
                      </button>
                      <button onClick={() => setShowHelpMenu(false)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-all text-left">
                        <Phone size={16} className="text-gray-600 dark:text-slate-400" />
                        <span className="text-sm text-gray-700 dark:text-slate-300 font-medium">(408) 775-7720</span>
                      </button>
                    </div>
                  </motion.div>
                )}
            ...
            <div className="relative" ref={companyMenuRef}>
              ...
              >
                <Building2 size={18} className="text-gray-600 dark:text-slate-400" />
                <ChevronDown size={14} className={cn('text-gray-400 dark:text-slate-500 transition-transform', showCompanyMenu && 'rotate-180')} />
              </motion.button>
              ...
              >
                <div className="text-right">
                  <div className="text-sm font-medium text-gray-900 dark:text-slate-100">{currentCompany?.name || 'Select Company'}</div>
                  <div className="text-xs text-gray-500 dark:text-slate-400">{currentCompany?.description || 'Company Account'}</div>
                </div>
                <ChevronDown size={16} className={cn('text-gray-400 dark:text-slate-500 transition-transform', showCompanyMenu && 'rotate-180')} />
              </motion.div>
              ...
              <AnimatePresence>
                {showCompanyMenu && (
                  ...
                  >
                    <div className="p-4 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900">
                      <div className="flex items-center gap-2">
                        <Building2 size={18} className="text-[#20319D] dark:text-[#6B8AFF]" />
                        <h3 className="text-sm font-semibold text-gray-900 dark:text-slate-100">Switch Company</h3>
                      </div>
                    ...
                    <div className="p-2 max-h-64 overflow-y-auto">
                      {companies.map((company) => {
                        ...
                        return (
                          <motion.button
                            key={company.id}
                            onClick={() => { switchCompany(company.id); setShowCompanyMenu(false); }}
                            className={cn(
                            ...
                            whileTap={{ scale: 0.98 }}
                          >
                            <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0', isActive ? 'bg-[#20319D]' : 'bg-gray-100 dark:bg-slate-700')}>
                              <Building2 size={16} className={isActive ? 'text-[#3BF493]' : 'text-gray-600 dark:text-slate-400'} />
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="text-sm font-medium text-gray-900 dark:text-slate-100 truncate">{company.name}</div>
                              <div className="text-xs text-gray-500 dark:text-slate-400">{company.description || 'Company Account'}</div>
                            </div>
                            {isActive && <Check size={16} className="text-[#20319D] flex-shrink-0" />}
                          </motion.button>
                        );
                      })}
                    </div>
                    ...
                    <div className="p-2">
                      <button
                        onClick={() => { router.push('/add-company'); setShowCompanyMenu(false); }}
                      ...
                      >
                        ...
                        <span className="text-sm text-gray-700 dark:text-slate-300 font-medium">Add Company</span>
                      </button>
                      <button
                        onClick={() => { router.push('/request-access'); setShowCompanyMenu(false); }}
                        className="w-full flex items-center gap-3 px-3 py-3 md:py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 active:bg-gray-100 dark:active:bg-slate-600 transition-all text-left min-h-[48px] md:min-h-0 touch-manipulation"
                      >
                        <Mail size={16} className="text-gray-600 dark:text-slate-400" />
                        <span className="text-sm text-gray-700 dark:text-slate-300 font-medium">Request Company Access</span>
                      </button>
                      <button
                        onClick={() => { router.push('/company-management'); setShowCompanyMenu(false); }}
                      ...
                      >
                        <Settings size={16} className="text-[#20319D] dark:text-[#6B8AFF]" />
                        <span className="text-sm text-[#20319D] dark:text-[#6B8AFF] font-medium">Company Management</span>
                      </button>
                    </div>
                  </motion.div>
                )}
            ...
            <div className="flex items-center gap-3 relative" ref={menuRef}>
              <div className="text-right hidden sm:block">
                <div className="text-sm font-medium text-gray-900 dark:text-slate-100">Hi, {user?.firstName || 'User'}</div>
                <span className="text-xs mt-0.5 px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-300">Admin</span>
              </div>
              <motion.div
                whileTap={{ scale: 0.95 }}
                  ...
                  >
                    ...
                    <div className="p-4 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-sm font-semibold text-gray-900 dark:text-slate-100">{userName}</div>
                          <div className="text-xs text-gray-500 dark:text-slate-400">{userEmail}</div>
                          <span className={cn('text-xs mt-1 px-2 py-0.5 rounded-full inline-block', userBadgeColor)}>
                            {userBadge}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="p-2">
                      ...
                      {userTypeOptions.map((option) => {
                        ...
                        return (
                          <motion.button
                            key={option.type}
                            onClick={() => { setUserType(option.type); setShowUserMenu(false); }}
                            className={cn(
                            ...
                            whileTap={{ scale: 0.98 }}
                          >
                            <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center', isActive ? 'bg-[#20319D] text-white' : 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-400')}>
                              <Icon size={16} />
                            </div>
                            ...
                            {isActive && <Check size={16} className="text-[#20319D]" />}
                          </motion.button>
                        );
                      })}
                    </div>
                    ...
                    {/* Menu Items */}
                    <div className="p-2">
                      <button onClick={() => { router.push('/profile'); setShowUserMenu(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-all text-left">
                      ...
                      <button onClick={() => { router.push('/company-management'); setShowUserMenu(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-all text-left">
                        <Building2 size={16} className="text-gray-600 dark:text-slate-400" />
                        <span className="text-sm text-gray-700 dark:text-slate-300">Company Management</span>
                      </button>
                      <button onClick={() => { router.push('/settings'); setShowUserMenu(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-all text-left">
                      ...
                      <button onClick={() => { router.push('/help'); setShowUserMenu(false); }} className="md:hidden w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-all text-left">
                        <HelpCircle size={16} className="text-gray-600 dark:text-slate-400" />
                        <span className="text-sm text-gray-700 dark:text-slate-300">Help & Support</span>
                      </button>

                      {/* Mobile-only: Dark Mode Toggle */}
                      <button
                        onClick={() => { setTheme(resolvedTheme === 'dark' ? 'light' : 'dark'); setShowUserMenu(false); }}
                      ...
                      >
                        <div className="flex items-center gap-3">
                          {resolvedTheme === 'dark' ? <Sun size={16} className="text-gray-600 dark:text-slate-400" /> : <Moon size={16} className="text-gray-600 dark:text-slate-400" />}
                          <span className="text-sm text-gray-700 dark:text-slate-300">{resolvedTheme === 'dark' ? 'Light Mode' : 'Dark Mode'}</span>
                        </div>
                        <div className={cn('w-9 h-5 rounded-full relative transition-colors', resolvedTheme === 'dark' ? 'bg-[#20319D]' : 'bg-gray-300 dark:bg-slate-600')}>
                          <div className={cn('absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-transform', resolvedTheme === 'dark' ? 'translate-x-4' : 'translate-x-0.5')} />
                        </div>
                      </button>
                      ...
                      <button
                        onClick={() => { signOut(); setShowUserMenu(false); router.push('/sign-in'); }}
      ...
      <AnimatePresence>
        {showMobileSearch && (
          ...
          >
            ...
            <div className="p-4">
              <p className="text-sm text-gray-500 dark:text-slate-400">Recent searches</p>
              <div className="mt-3 space-y-2">
                <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800 transition-all text-left">
                  ...
                  <span className="text-sm text-gray-700 dark:text-slate-300">Invoice #1234</span>
                </button>
                <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800 transition-all text-left">
                  <Search size={16} className="text-gray-400 dark:text-slate-500" />
                  <span className="text-sm text-gray-700 dark:text-slate-300">ABC Suppliers</span>
                </button>
              </div>
            </div>
          </motion.div>
